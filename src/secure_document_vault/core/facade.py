...
def encriptar(
    ...
    :param signer_keystore: Keystore dict del firmante
    :param signer_password: Password del firmante
    :return: Archivo completo en formato .vault como bytes.
    """
    dev2_randomness = RandomnessManager()
    dev3_builder = VaultBuilder()

    signer_private_key = KeyProtector.verify_password(signer_password, signer_keystore)
    try:
...
def desencriptar(
    ...
    private_key = KeyProtector.verify_password(user_password, user_keystore)
    try:
