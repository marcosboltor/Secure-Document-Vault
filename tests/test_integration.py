...
def test_multiple_recipients_all_can_decrypt():
    ...
    vault = encriptar(plaintext, "group.txt", users)

    for user in users:
        recovered = desencriptar(vault, user["id"], user["private_key"])
...
def test_multiple_recipients_non_member_cannot_decrypt():
    ...
    vault = encriptar(plaintext, "private.txt", users)

    with pytest.raises(IntegrityErrorException, match="no autorizado"):
        desencriptar(vault, outsider["id"], outsider["private_key"])
...
def test_encrypt_decrypt_empty_file():
    ...
    vault = encriptar(plaintext, "empty.txt", [alice])
...
def test_encrypt_decrypt_binary_content():
    ...
    vault = encriptar(plaintext, "document.pdf", [alice])
    recovered = desencriptar(vault, "alice", alice["private_key"])
...
def test_encrypt_decrypt_large_file():
    ...
    vault = encriptar(plaintext, "large_file.dat", [alice])
    recovered = desencriptar(vault, "alice", alice["private_key"])
...
def test_single_recipient_encrypt_decrypt():
    ...
    vault = encriptar(plaintext, "alice_only.txt", [alice])
    recovered = desencriptar(vault, "alice", alice["private_key"])
...
def test_single_recipient_another_user_rejected():
    ...
    with pytest.raises(IntegrityErrorException, match="no autorizado"):
        desencriptar(vault, bob["id"], bob["private_key"])
...
def test_vault_is_reusable_multiple_decryptions():
    ...
    vault = encriptar(plaintext, "reusable.txt", [alice, bob])

    for _ in range(3):
        assert desencriptar(vault, "alice", alice["private_key"]) == plaintext
        assert desencriptar(vault, "bob", bob["private_key"]) == plaintext
...
def test_vault_metadata_contains_filename():
    ...
    vault = encriptar(b"Content", filename, [alice])
...
def test_two_encryptions_of_same_plaintext_differ():
    ...
    vault1 = encriptar(plaintext, "file.txt", [alice])
    vault2 = encriptar(plaintext, "file.txt", [alice])

    assert vault1 != vault2
    # Both can still be decrypted correctly
    assert desencriptar(vault1, "alice", alice["private_key"]) == plaintext
    assert desencriptar(vault2, "alice", alice["private_key"]) == plaintext
...
def test_invalid_header_raises_value_error():
    ...
    with pytest.raises(ValueError, match="Header incorrecto"):
        desencriptar(fake_vault, "alice", alice["private_key"])
...
def test_empty_bytes_raises_error():
    ...
    with pytest.raises(Exception):
        desencriptar(b"", "alice", alice["private_key"])
...
def test_random_bytes_not_valid_vault():
    ...
    with pytest.raises(Exception):
        desencriptar(garbage, "alice", alice["private_key"])
...
def test_tampered_wrapped_key_fails():
    ...
    vault = encriptar(plaintext, "doc.txt", [alice])
...
def test_encrypt_decrypt_special_filename(nombre):
    ...
    vault = encriptar(plaintext, nombre, [alice])
    recovered = desencriptar(vault, "alice", alice["private_key"])
...
def test_end_to_end_file_types(nombre, contenido):
    ...
    vault = encriptar(contenido, nombre, [alice, bob])

    assert desencriptar(vault, "alice", alice["private_key"]) == contenido
    assert desencriptar(vault, "bob", bob["private_key"]) == contenido
...
def test_removing_recipient_entry_breaks_access():
    ...
    vault = encriptar(plaintext, "shared.txt", [alice, bob])
    ...
    with pytest.raises(IntegrityErrorException, match="no autorizado"):
        desencriptar(tampered_vault, "bob", bob["private_key"])

    # Alice cannot decrypt either: the AAD changed so the AEAD tag no longer matches
    with pytest.raises(IntegrityErrorException):
        desencriptar(tampered_vault, "alice", alice["private_key"])
