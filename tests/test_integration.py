...
def test_multiple_recipients_all_can_decrypt():
    ...
    vault = encriptar(plaintext, "group.txt", users, users[0]["id"], users[0]["signing_private_key"])

    for user in users:
        recovered = desencriptar(vault, user["id"], user["private_key"], users[0]["signing_public_key"])
...
def test_multiple_recipients_non_member_cannot_decrypt():
    ...
    vault = encriptar(plaintext, "private.txt", users, users[0]["id"], users[0]["signing_private_key"])

    with pytest.raises(IntegrityErrorException, match="no autorizado"):
        desencriptar(vault, outsider["id"], outsider["private_key"], users[0]["signing_public_key"])
...
def test_encrypt_decrypt_empty_file():
    ...
    vault = encriptar(plaintext, "empty.txt", [alice], alice["id"], alice["signing_private_key"])
    recovered = desencriptar(vault, "alice", alice["private_key"], alice["signing_public_key"])
...
def test_encrypt_decrypt_binary_content():
    ...
    vault = encriptar(plaintext, "document.pdf", [alice], alice["id"], alice["signing_private_key"])
    recovered = desencriptar(vault, "alice", alice["private_key"], alice["signing_public_key"])
...
def test_encrypt_decrypt_binary_with_null_bytes():
    ...
    vault = encriptar(plaintext, "binary.bin", [alice], alice["id"], alice["signing_private_key"])
    recovered = desencriptar(vault, "alice", alice["private_key"], alice["signing_public_key"])
...
def test_encrypt_decrypt_large_file():
    ...
    vault = encriptar(plaintext, "large_file.dat", [alice], alice["id"], alice["signing_private_key"])
    recovered = desencriptar(vault, "alice", alice["private_key"], alice["signing_public_key"])
...
def test_single_recipient_encrypt_decrypt():
    ...
    vault = encriptar(plaintext, "alice_only.txt", [alice], alice["id"], alice["signing_private_key"])
    recovered = desencriptar(vault, "alice", alice["private_key"], alice["signing_public_key"])
...
def test_vault_is_reusable_multiple_decryptions():
    ...
    vault = encriptar(plaintext, "reusable.txt", [alice, bob], alice["id"], alice["signing_private_key"])

    for _ in range(3):
        assert desencriptar(vault, "alice", alice["private_key"], alice["signing_public_key"]) == plaintext
        assert desencriptar(vault, "bob", bob["private_key"], alice["signing_public_key"]) == plaintext
...
def test_vault_metadata_contains_filename():
    ...
    vault = encriptar(b"Content", filename, [alice], alice["id"], alice["signing_private_key"])
...
def test_vault_metadata_contains_all_recipient_ids():
    ...
    vault = encriptar(b"Data", "multi.txt", recipients, alice["id"], alice["signing_private_key"])
...
def test_vault_metadata_algorithm_field():
    ...
    alice = create_user("alice")
    vault = encriptar(b"Test", "test.txt", [alice], alice["id"], alice["signing_private_key"])
...
def test_two_encryptions_of_same_plaintext_differ():
    ...
    assert desencriptar(vault1, "alice", alice["private_key"], alice["signing_public_key"]) == plaintext
    assert desencriptar(vault2, "alice", alice["private_key"], alice["signing_public_key"]) == plaintext
...
def test_invalid_header_raises_value_error():
    ...
    with pytest.raises(ValueError, match="Header incorrecto"):
        desencriptar(fake_vault, "alice", alice["private_key"], alice["signing_public_key"])
...
def test_random_bytes_not_valid_vault():
    ...
    with pytest.raises(Exception):
        desencriptar(garbage, "alice", alice["private_key"], alice["signing_public_key"])
...
def test_each_recipient_has_unique_wrapped_key():
    ...
    vault = encriptar(b"Shared", "shared.txt", [alice, bob], alice["id"], alice["signing_private_key"])
...
def test_encrypt_decrypt_special_filename(nombre):
    ...
    vault = encriptar(plaintext, nombre, [alice], alice["id"], alice["signing_private_key"])
    recovered = desencriptar(vault, "alice", alice["private_key"], alice["signing_public_key"])
...
def test_end_to_end_file_types(nombre, contenido):
    ...
    vault = encriptar(contenido, nombre, [alice, bob], alice["id"], alice["signing_private_key"])

    assert desencriptar(vault, "alice", alice["private_key"], alice["signing_public_key"]) == contenido
    assert desencriptar(vault, "bob", bob["private_key"], alice["signing_public_key"]) == contenido
...
def test_removing_recipient_entry_breaks_access():
    ...
    vault = encriptar(plaintext, "shared.txt", [alice, bob], alice["id"], alice["signing_private_key"])
    ...
    with pytest.raises(IntegrityErrorException, match="Firma digital"):
        desencriptar(tampered_vault, "bob", bob["private_key"], alice["signing_public_key"])

    # Alice cannot decrypt either: the AAD changed so the AEAD tag no longer matches
    with pytest.raises(IntegrityErrorException):
        desencriptar(tampered_vault, "alice", alice["private_key"], alice["signing_public_key"])
