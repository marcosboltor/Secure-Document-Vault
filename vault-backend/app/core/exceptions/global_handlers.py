...
def setup_exception_handlers(app: FastAPI):
    ...
    async def app_exception_handler(request: Request, exc: AppBaseException):
        """Manejador para excepciones controladas de la aplicación (Dominio e Infraestructura)."""
        logger.warning(f"AppException [{exc.code}]: {exc.message} at {request.url.path}")
        return JSONResponse(
