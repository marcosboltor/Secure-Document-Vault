...
export default function UploadPage() {
  const [stagingQueue, setStagingQueue] = useState([
    ...
    { name: "Employee_Database_...", size: "1.2 GB • Pending", progress: 0, icon: <div style={{ opacity: 0.5 }}><FileText size={18} /></div> },
    { name: "Master_Encryption_K...", size: "4 KB • Pending", progress: 0, icon: <Lock size={18} /> },
  ]);

  const removeFile = (index: number) => {
    setStagingQueue(prev => prev.filter((_, i) => i !== index));
  };
    ...
    <div className={styles.container}>
      ...
      <aside className={styles.queueSidebar}>
        <div className={styles.queueHeader}>
          ...
          <span className={styles.itemCount}>{stagingQueue.length} Items</span>
        </div>
          ...
          {stagingQueue.map((item, i) => (
            <div key={i} className={styles.queueItem}>
              <div className={styles.itemMain}>
                ...
                <X size={16} className={styles.closeIcon} onClick={() => removeFile(i)} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={styles.queueFooter}>
          ...
          <button className={styles.transferBtn}>
            <ShieldCheck size={18} /> Commence Transfer
          </button>
        </div>
      </aside>
    </div>
